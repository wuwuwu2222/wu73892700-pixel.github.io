...
const PortraitPhotos: React.FC = () => {
  ...
  const portraitImages = useMemo(() => {
    return processedImageData.filter(image => image.category === 'portrait');
  }, []);

  const visibleImages = useMemo(() => {
    return portraitImages.slice(0, visibleCount);
  }, [portraitImages, visibleCount]);

  const loadMore = useCallback(() => {
    if (loadingRef.current || visibleCount >= portraitImages.length) return;
    loadingRef.current = true;
    setVisibleCount(prev => Math.min(prev + 4, portraitImages.length));
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && visibleCount < portraitImages.length) {
          loadMore();
        }
      },
      { threshold: 0.1 }
  ...
  }, [visibleCount, portraitImages.length, loadMore]);

  const getSelectedImage = useCallback(() => {
    const image = portraitImages.find(i => i.id === selectedImage);
    if (image) {
      // 为图片路径添加 base 前缀
    ...
    return undefined;
  }, [portraitImages, selectedImage, base]);
    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="container mx-auto px-4 pt-24 pb-16">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <h1 className="text-4xl font-bold mb-8 text-center text-zinc-900">人像摄影</h1>
          <p className="text-center text-zinc-600 mb-12 max-w-2xl mx-auto">人像摄影作品，捕捉人物的魅力与情感</p>
